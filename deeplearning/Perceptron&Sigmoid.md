...
Geometric interpretation \
\^y_{i} =  \sum_{i=1}^{n} \omega_{i} x_{i} \geq b in 2D can be rewritten as
...
So if  $$\omega.x < 0$$\
we adjust w such that $$\omega = \omega+x $$\
It means that angle between them is greater than 90, but we want it to be less than 90. Doing this will get the new angle between w and x to be decreased.

## Sigmoid Neuron 

They are similar to Perceptron model but are modified such that the output is much smoother than the harsh decision boundary from perceptron.\
...
## Update rule ..
 ...
 $$L(\theta + \eta u ) = L(\theta) + \eta * u^{T} \Delta_{t} L(\theta)$$\
 This would work only if 
 
 $$u^{T} \Delta_{t} L(\theta)<0 $$
